...
class DirTree:
    ...
    def __init__(self, rootpath="",isStop=None):
        ...
        DirTree constructor.
        """
        self.rootpath = os.path.basename(rootpath)
    ...
    def read_disk(self, pathlist, depth, callback_dir=None ):
        ...
        #print self.et.tag
        if pathlist:
            self.rootpath = os.path.basename(pathlist[0])
    ...
    def read_sent_file(self, filename,destPath):
        """
        ...
        if "AROWsynchro.xml" in filename:
            ...
            for f in path(self.rootpath).files("AROWsynchro_*"):
                ...
                if not found:
                    os.remove(f.name)
        return flist       
    def read_rcvd_file(self, filename):
        """
        to read the DirTree from an XML file.
        """
        flist=[]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print (__doc__)
        print (u"usage: python %s <root path> <xml file> [previous xml file]" % os.path.basename(sys.argv[0]).name)
